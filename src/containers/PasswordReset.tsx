...
export default withRouter(function PasswordReset({ history, location, match }) {
  ...
  const onSubmit = (e: FormEvent) => {
    ...
    if (!errorOccured) {
      ...
      apis
        .changePassword(token, password1)
        .then((response) => {
          alert("비밀번호를 변경했습니다.");
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={12}>
            <form onSubmit={onSubmit}>
              ...
              />
              <ul>
                <li>비밀번호 확인은 비밀번호와 일치해야 합니다.</li>
              </ul>
              <Button type="submit" variant="contained" color="primary" fullWidth size="large" className={classes.loginBtn} disabled={resetBtnDisabled}>
                변 경
              </Button>
              <br />
            </form>
